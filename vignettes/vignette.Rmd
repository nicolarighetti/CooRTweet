...
We introduce `CooRTweet` an R-Package for detecting coordinated behavior on social media. 
...
The package is compatible with any social media, as long as the data set contains the required variables. It offers native support for the Twitter Academic API V2 in JSON format and includes a convenience function (`prep_data`) for preparing other types of data in the format necessary for the package.
...
The `detect_groups()` function enables the identification of users who shared the same objects (denoted as `object_id`) within a predefined time interval, `time_window` (represented by $\tau$). Additionally, the function includes a parameter, `min_participation` (denoted as $r$, corresponding to the `degree` metric in Social Network Analysis), which allows for filtering users based on a minimum participation threshold in the network.

This function returns a data.table object, which is subsequently processed by the `generate_coordinated_network` function. This function completes the final stage of coordinated analysis. It involves filtering users who performed identical actions within the same timeframe, in accordance with the degree of repetition. The underlying assumption is that two users may coincidentally share the same objects within the same time window; however, the likelihood of them repeatedly sharing the same object within the same time window is considerably lower [@Giglietto2020]. The degree metric serves to operationalize the concept of repetition. Furthermore, the function computes an `edge_symmetry_score`, which aids in evaluating the impact of the number of shares contributed by each user on the edge.
...
-   `account_id`: the unique ID of a twitter user.
...
The coordinated detection is then completed by applying the other function. We set the "objects" option to TRUE so that the graph keeps the list of objects shared by users, for later inspection via the `group_stats` function. We also set a filter on the graph that identifies edges with a weight greater than 99% of the edges weight in the graph. This is used to identify users who repeatedly share `object_id` (i.e, any type of identified content) in the same `time_window`.
...
We can quickly get some summary statistics by using the provided convenience functions `group_stats()` and `user_stats()`. If we are interested in the content that users share in a coordinated fashion, we can call `group_stats()` and pass in our `igraph` object from the `generate_coordinated_network` function:

```{r results='hide'}
summary_groups <- group_stats(coord_graph, network = "full")
```

`summary_groups` shows how many users (column `num_users`) participated for each unique shared object (`object_id`).

If you are interested in understanding more about the users you can call `user_stats()`:

```{r results='hide'}
summary_users <- user_stats(coord_graph, weight_threshold = "full")
```

`summary_users` shows that the user with the ID `017ae089cdee16bb4a240a0547457ed4` shared a total of 6 coordinated posts and within an average time window of 104.16 seconds (evaluated on the network filtered by edge_weight, as indicated by the `weight_threshold = "full"` option. The documentation for each function includes details and possible options).
...
Using special options of the `generate_coordinated_network` function, we can get the graph of users who have shared content faster (`subgraph = 2`). Other options allow for the general network filtered by edge weight (`subgraph = 1`) or the fastest network unfiltered by edge weight (`subgraph = 3`).
...
There are two functions that give summaries of the `igraph` data resulting from the `generate_coordinated_network` function: `group_stats()` and `user_stats()`.
...
It returns a `data.table` which shows the group statistics for total count of unique users that shared that object. 

If you are interested in the user statistics, you can pass the `igraph` resulting from `generate_coordinated_network` into `user_stats()`. Depending on whether you want statistics for the general network, or for the fastest network, if it has been calculated via the `flag_speed_share` function, you can spefic "fast," or you need to specify "full," or "none," in the "weight_threshold" argument.

```{r results='hide', eval=FALSE}
summary_users <- user_stats(result, weight_threshold = "fast")
```

It provides summary statistics for each user in the network: total coordinated posts shared (`content_id`), and average time delta (more specifically, this value represents the average of the mean time_delta values of each user). High number of posts shared and low average time delta might suggest highly coordinated (and potentially automated) user behavior.
